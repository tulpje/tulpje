    ...
    S: Unpin + Sync + Send + Serialize + for<'de> Deserialize<'de> + Default,
{
    ...
    let state = interaction_state::get(&ctx.services.db, guild_id, message_id)
        .await?
        .ok_or_else(|| {
    ...
    S: Unpin + Sync + Send + Serialize + for<'de> Deserialize<'de>,
{
    ...
    match step.run(ctx, state).await? {
        Some(state) => {
            // store the new state
            interaction_state::set(&db, guild_id, message_id, state)
                .await
        ...
        None => {
            ...
            interaction_state::delete(&db, guild_id, message_id)
                .await
